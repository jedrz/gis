...
Dostępne są również lista przechowująca wszystkie wierzchołki grafu i
analogiczna lista przechowująca krawędzie. Wynik algorytmu przechowywany jest
wraz z grafem w osobnej strukturze co ma na celu uproszczenie implementacji
interfejsu użytkownika.

Najciekawszą strukturą w naszym projekcie jest \textbf{las}. Używamy jej do
przechowywania przejść algorytmu DFS w naszych algorytmach. Jest to lista
drzew, gdzie każde drzewo jest jednym przejściem algorytmu DFS. Poniżej
przykład grafu i odpowiedniego mu drzewa (przy założeniu, że zaczynamy od
wierzchołka nr 1):
...
Na potrzeby projektu powstała również lista grafów, która służy do
przechowywania grafów utworzonych ręcznie na potrzeby testów jednostkowych oraz
prezentacji możliwości algorytmu w interfejsie użytkownika.
